...
app.config['SQLALCHEMY_DATABASE_URI'] = 'sqlite:///app.db' # Changed from sites.db to app.db for consistency
...
def load_user(user_id):
    ...
    This function is crucial for Flask-Login to re-authenticate users on subsequent requests.
    It now attempts to load both regular Users and Admins.
    """
    # First, try to load a regular User
    user = User.query.get(int(user_id))
    if user:
        return user
    
    # If not found as a regular User, try to load as an Admin
    admin = Admin.query.get(int(user_id))
    if admin:
        return admin
    
    # If neither is found, return None (user is anonymous)
    return None
...
# --- Helper function for allowed file extensions (Profile Pics) ---
def allowed_file(filename):
    return '.' in filename and \
           filename.rsplit('.', 1)[1].lower() in ALLOWED_EXTENSIONS

# --- NEW: Helper function for allowed Blog media file extensions ---
...
# --- NEW: Helper function for allowed Event media file extensions ---
...
# In app.py, remove 'is_admin' from your User model:
...
class Admin(db.Model, UserMixin):
    ...
    @property
    def is_active(self):
        return True # Or implement logic for account activation/deactivation

    @property
    def is_authenticated(self):
        # This will be True if an Admin object is logged in
        return True
...
# Decorator to ensure only authenticated admins can access a route (provided by user)
def admin_required(f):
    @wraps(f)
    def decorated_function(*args, **kwargs):
        # Ensure the user is authenticated AND it's an instance of the Admin model
...
# --- Menstrual Health Tracking Models ---
...
class BlogPost(db.Model):
    ...
    image_filename = db.Column(db.String(200), nullable=True) # NEW: For blog post image
    video_filename = db.Column(db.String(200), nullable=True) # NEW: For blog post video
    is_approved = db.Column(db.Boolean, default=False, nullable=False) # NEW: For admin approval
...
def send_email(to_email, subject, body):
    ...
    sender_password = os.environ.get('EMAIL_PASS', 'qjju lypl enik damd')
...
def signup():
    if request.method == 'POST':
        ...
        try:
            ...
            welcome_subject = "Welcome to The Gutsy Woman Network!"
            welcome_body = f"Dear {full_name},\n\nWelcome to The Gutsy Woman Network! We're thrilled to have you join our community of impactful women.\n\nBest regards,\nThe Gutsy Woman Team"
